...
func (repo *Repository) CreateMessage(message models.Message) (models.Message, error) {
	...
	err = result.Scan(&message.ID, &message.CreatedAt)
	if err != nil {
		return models.Message{}, err
	}
...
func (repo *Repository) CreateRoom(room models.Room) (models.Room, error) {
	...
	OUTPUT INSERTED.id, INSERTED.created_at
	VALUES (@name, @is_private, @created_by)`)
	if err != nil {
		return models.Room{}, err
	}
	defer stmt.Close()
	...
	err = result.Scan(&room.ID, &room.CreatedAt)
	if err != nil {
		return models.Room{}, err
	}

	stmt, err = tx.Prepare(`INSERT INTO room_members (room_id, user_id) VALUES (@room_id, @user_id)`)
	if err != nil {
		return models.Room{}, err
	}
	defer stmt.Close()

	_, err = stmt.Exec(sql.Named("room_id", room.ID), sql.Named("user_id", room.CreatedBy))
	if err != nil {
		return models.Room{}, err
	}
